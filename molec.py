    ...
    def setParams(self):
        ...
        wlinc = {'vis': [[0.686, 0.694], [0.758, 0.762], [0.762, 0.770],
            ...
            'nir': [[1.12, 1.13], [1.14, 1.15], [1.47, 1.48], [1.48, 1.49],
                    [1.80, 1.81]]}
        # wlinc = {'vis': [[0.758, 0.762], [0.762, 0.770],
        #              [0.940, 0.951]],
        #     'nir': [[1.12, 1.13], [1.14, 1.15]]}
    ...
    def runMolec(self):
        ...
        with open(runtranspar, 'w') as f:
            runMolec = subprocess.run([transCall, self.molecparfile],
                                        stdout=f)
        f.close()
        with open(runtranspar, 'r') as f:
            lines = f.read().splitlines()
            runTransRes = lines[-1].strip()

        if runTransRes == '[ INFO  ] No errors occurred':
